...
export default function Signup() {
    const arr = JSON.parse(localStorage.getItem('E-commerce')) || []
    console.log(arr);
    let nameRef = useRef()
    let emailRef = useRef()
    ...
    const handelSignup = (e) => {
        ...
        if (!obj.name) {
            return toast.warn('Incomplete information! Please fill in all required fields.', {
        ...
        localStorage.setItem('E-commerce', JSON.stringify(arr))
         navigate('/login')
        toast.success('Account created! Login to start shopping', {
            position: "top-right",
            theme: "dark",
        })
    }
            ...
            <div className="font-[sans-serif] bg-white max-w-4xl flex items-center mx-auto md:h-screen p-4">
                <div className="grid md:grid-cols-3 items-center shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-xl overflow-hidden">
                    ...
                    <form className="md:col-span-2 w-full py-6 px-6 sm:px-16">
                        ...
                        <div className="!mt-12">
                            <button onClick={handelSignup} type="button" className="w-full py-3 px-4 tracking-wider text-sm rounded-md text-white bg-gray-700 hover:bg-gray-800 focus:outline-none">
                                Create an account
                            </button>
                        </div>
